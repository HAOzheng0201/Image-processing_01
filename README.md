...
各题 README 是学习说明，可以帮助组织报告，但不能替代题目要求的报告 PDF。报告中应交代运行方法、实现原理、核心代码和主要代表性结果，尤其是第 3 题两种灰度方法的差异，以及第 6 题库函数和边界方式的比较。

“运行方法”应说明在哪个目录、使用哪个环境、执行什么命令，不能只描述程序内部的处理流程。报告开头可统一交代：在 `01_homework` 根目录激活 `imageproc_hw01`；各题再给出对应的 `python questionXX/main.py` 命令。第 6 题还需保留第 3、4、5 题的代码目录，但不需要先运行它们。

整理报告时，几个结论需要限定适用范围：

- 第 3 题的两种灰度方法差异不等于质量评价；可结合具体颜色区域和浮点差异数值说明。
- 第 6 题核半径为 15 时，可能受填充影响的是四周宽 15 像素的边界带，内部为 `[15:-15,15:-15]`。
- 第 7 题的重建一致验证像素可恢复；块顺序还应结合指定块与原图坐标切片的比较。代码已包含这两类检查。

报告中的示例路径和软件包接口应与提交的程序一致。核心代码使用截图时，导出 PDF 后要在正常阅读比例下检查字号、清晰度和是否被裁切；文字提取无法代替这一步版式检查。
